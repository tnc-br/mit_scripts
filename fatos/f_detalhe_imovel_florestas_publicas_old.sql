...
with analise_espacial as (
	select 	B.id_imovel as id_imovel,
			C.id_florestas_publicas as id_floresta_publica,
			st_area(A.geom) as area_calc_imovel,
			st_area(C.geom) as area_calc_floresta_publica,
			st_area(processado.st_union_or_ignore(st_intersection(A.geom, C.geom))) as area_over_floresta_publica,
...
select 	--nextval('processado.f_detalhe_imovel_florestas_publicas_seq') as id_f_detalhe_imovel_florestas_publicas,
		...
		area_calc_floresta_publica,
		area_calc_floresta_publica / 10000 as area_calc_floresta_publica_ha,
		area_over_floresta_publica,
		area_over_floresta_publica / 10000 as area_over_floresta_publica_ha,
		round((area_over_floresta_publica / area_calc_imovel)::numeric,4) as perc_over_floresta_publica,
		data_ultima_analise
